...
/*
    ...
    8.分类:
        1.内部函数: 在函数的返回值前加static, 其他文件不能访问. 如果static 写在函数的实现中, 代表定义一个内部函数. 如果static 写在函数的声明中, 代表声明一个内部函数. [在.h 中声明内部函数没有意义]
        2.外部函数: 只要在函数的返回之前加一个extern, 就可以让函数变为一个外部函数. 由于默认就是外部函数, 因此一般不写extern. 如果extern 写在函数的实现中, 代表定义一个外部函数. 如果extern 写在函数的声明中, 代表声明一个外部函数.
 */
